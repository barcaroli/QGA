...
functionMax <- function(solution, eval_func_inputs) {
  ...
  Genome   <- eval_func_inputs$Genome  # <-- passa Genome come input!
  
  # traduci il bitstring (0/1) in intero
  x <- 0
  for (j in seq_len(Genome)) {
    x <- x + solution[j] * 2^(Genome - j)
  }
  
  # normalizza (NB: questo dà range [-1, 15])
  x <- x * 16 / (2^Genome - 1) - 1
...
```{r}
set.seed(1234)
solutionQGA <- QGA(popsize               = popsize,
                   generation_max        = generation_max,
                   nvalues_sol           = nvalues_sol,
                   Genome                = Genome,
                   thetainit             = thetainit,
                   thetaend              = thetaend,
                   pop_mutation_rate_init= pop_mutation_rate_init,
                   pop_mutation_rate_end = pop_mutation_rate_end,
                   ...
                   eval_func_inputs      = list(Genome = Genome))
```
